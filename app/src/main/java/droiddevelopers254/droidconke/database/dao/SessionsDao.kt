...
interface SessionsDao {
    ...
    suspend fun getDayOneSessions(dayNumber: String): LiveData<List<SessionsModel>>

    @Query("SELECT * FROM sessionsList WHERE day_number =:dayNumber")
    suspend fun getDayTwoSessions(dayNumber: String): LiveData<List<SessionsModel>>

    @Query("SELECT * FROM sessionsList WHERE day_number =:dayNumber AND id =:sessionId")
    suspend fun getSessionDetails(dayNumber: String, sessionId: Int): LiveData<SessionsModel>
    ...
    suspend fun isSessionStarred(sessionId: Int, dayNumber: String): LiveData<Int>
