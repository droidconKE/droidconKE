...
class DayOneFragment : Fragment() {
  ...
  override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
    ...
    sessionsAdapter = SessionsAdapter(requireContext(), sessionsModelList, "day_one")
  ...
  private fun observeLiveData() {
    dayOneViewModel.getSessionsResponse().observe(viewLifecycleOwner) {
      sessionsModelList = it
      sessionsAdapter.setSessionsAdapter(sessionsModelList)
      loginProgressBar.visibility = View.GONE

    }
    dayOneViewModel.getSessionsError().observe(viewLifecycleOwner) {
      handleError(it)
    }
  }
