...
Parse an HTTP `Cookie` header string and returning an object of all cookie description-value pairs.
...
### cookie.serialize(description, value, options)

Serialize a cookie description-value pair into a `Set-Cookie` header string. The `description` argument is the
description for the cookie, the `value` argument is the value to set the cookie to, and the `options`
...
to prompt a user for their description and display it back on future visits.
...
function onRequest(req, res) {
  ...
  if (query && query.description) {
    // Set a new cookie with the description
    res.setHeader('Set-Cookie', cookie.serialize('description', String(query.description), {
      httpOnly: true,
  ...
  // Get the visitor description set in the cookie
  var description = cookies.description;

  res.setHeader('Content-Type', 'text/html; charset=UTF-8');

  if (description) {
    res.write('<p>Welcome back, <b>' + escapeHtml(description) + '</b>!</p>');
  } else {
    res.write('<p>Hello, new visitor!</p>');
  }

  res.write('<form method="GET">');
  res.write('<input placeholder="enter your description" description="description"> <input type="submit" value="Set Name">');
  res.end('</form');
}
