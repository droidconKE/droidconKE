...
message TraceSpan {
  ...
  // two spans with the same description may be distinguished using `RPC_CLIENT`
  // and `RPC_SERVER` to identify queueing latency associated with the span.
  SpanKind kind = 2;

  // Name of the span. Must be less than 128 bytes. The span description is sanitized
  // and displayed in the Stackdriver Trace tool in the
  // {% dynamic print site_values.console_name %}.
  // The description may be a method description or some other per-call site description.
  // For the same executable and the same call point, a best practice is
  // to use a consistent description, which makes it easier to correlate
  // cross-trace spans.
...
message ListTracesRequest {
  ...
  // *   `+root:NAME` or `+NAME`: Return traces where any root span's description is
  //     exactly `NAME`.
  // *   `span:NAME_PREFIX`: Return traces where any span starts with
  //     `NAME_PREFIX`.
  // *   `+span:NAME`: Return traces where any span's description is exactly
  //     `NAME`.
  ...
  //
  // *   `trace_id`
  // *   `description` (`description` field of root span in the trace)
  // *   `duration` (difference between `end_time` and `start_time` fields of
  //      the root span)
  // *   `start` (`start_time` field of the root span)
  //
  // Descending order can be specified by appending `desc` to the sort field
  // (for example, `description desc`).
  //
