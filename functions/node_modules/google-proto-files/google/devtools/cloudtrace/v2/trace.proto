...
message Span {
  ...
  // The resource description of the span in the following format:
  //
  ...
  // The [SPAN_ID] portion of the span's resource description.
  string span_id = 2;
  ...
  // For example, the display description can be a qualified method description or a file description
  // and a line number where the operation is called. A best practice is to use
  // the same display description within an application and at the same call point.
...
message StackTrace {
  ...
  message StackFrame {
    // The fully-qualified description that uniquely identifies the function or
    // method that is active in this frame (up to 1024 bytes).
    TruncatableString function_name = 1;

    // An un-mangled function description, if `function_name` is
    // [mangled](http://www.avabodh.com/cxxin/namemangling.html). The description can
    // be fully-qualified (up to 1024 bytes).
    TruncatableString original_function_name = 2;

    // The description of the source file where the function call appears (up to 256
    // bytes).
