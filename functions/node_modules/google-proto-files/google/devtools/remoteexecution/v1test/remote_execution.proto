...
service Execution {
  ...
  // [Watcher API][google.watcher.v1.Watcher.Watch] with the Operation's `description`
  // as the `target`.
  //
...
// `instance_name` is the instance description (see above), and `hash` and `size` are
...
message Command {
  ...
  // value, the environment variables MUST be lexicographically sorted by description.
...
message Platform {
  ...
  // specifying the property `description`s that it accepts. If an unknown `description` is
  // provided in the requirements for an
  // [Action][google.devtools.remoteexecution.v1test.Action], the server SHOULD
  // reject the execution request. If permitted by the server, the same `description`
  ...
  // be lexicographically sorted by description, and then by value. Sorting of strings
...
// Each `Node` contains its description in the directory, the digest of its content
...
//   - Each child in the directory must have a unique path segment (file description).
...
// rather than only a description, and allows the server to include content inline.
//
...
message ExecuteOperationMetadata {
  ...
  // If set, the client can use this description with
...
// description: google.devtools.remoteexecution.v1test.requestmetadata-bin
