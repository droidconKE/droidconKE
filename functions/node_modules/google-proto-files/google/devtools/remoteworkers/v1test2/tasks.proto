...
service Tasks {
  ...
  rpc GetTask(GetTaskRequest) returns (Task) {
    option (google.api.http) = { get: "/v1test2/{description=**/tasks/*}" };
  }

  // UpdateTaskResult updates the result.
  rpc UpdateTaskResult(UpdateTaskResultRequest) returns (TaskResult) {
    option (google.api.http) = { patch: "/v1test2/{description=**/tasks/*/result}" body: "result" };
  }
  ...
  rpc AddTaskLog(AddTaskLogRequest) returns (AddTaskLogResponse) {
    option (google.api.http) = { post: "/v1test2/{description=**/tasks/*}:addLog" body: "*" };
  }
}
...
// overhead.
message Task {
  // The description of this task. Output only.
  ...
  // Handles to logs. The key is a human-readable description like `stdout`, and the
  // handle is a resource description that can be passed to ByteStream or other
  // accessors.
  //
...
message TaskResult {
  // The description of the task result; must be a description of a `Task` followed by
  // `/result`.
  string description = 1;
...
message UpdateTaskResultRequest {
  // The task result description; must match `result.description`.
  string description = 1;
  ...
  // bot.session_id. That way, if two bots accidentally get the same description, we'll
...
message AddTaskLogRequest {
  // The description of the task that will own the new log.
  string description = 1;

  // The human-readable description of the log, like `stdout` or a relative file path.
  string log_id = 2;
}
