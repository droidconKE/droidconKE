...
service Bots {
  ...
  // using the same description accidentally.
  ...
  rpc UpdateBotSession(UpdateBotSessionRequest) returns (BotSession) {
    option (google.api.http) = { patch: "/v1test2/{description=**/botSessions/*}" body: "bot_session" };
  }
  ...
  rpc PostBotEventTemp(PostBotEventTempRequest) returns (google.protobuf.Empty) {
    option (google.api.http) = { post: "/v1test2/{description=**/botSessions/*}:postEvent" body: "*" };
  }
}
...
message BotSession {
  // The bot session description, as selected by the server. Output only during a call
  // to CreateBotSession.
  ...
  // farm. Typically, the bot ID will be the same as the description of the primary
...
message Lease {
  ...
  // solely on this description, either because the bot only understands one type of
  ...
  // resource would be the description of the queue, something like
  ...
  //
  // Note that the assignment may be a [full resource description]
  ...
  // While the `assignment` field is a resource description that allows the bot to
...
message Device {
  ...
  // The handle can be thought of as the "description" of the device, and must be
  // unique within a Worker.
  //
  // In the Status context, the handle should be some human-understandable description,
  // perhaps corresponding to a label physically written on the device to make
  // it easy to locate. In the Request context, the description should be the
  // *logical* description expected by the task. The bot is responsible for mapping the
  // logical description expected by the task to a machine-readable description that the task
...
message AdminTemp {
  ...
  enum Command {
    ...
    // Shut down the bot. `arg` will be a task resource description (similar to those
...
message CreateBotSessionRequest {
  ...
  // The bot session to create. Server-assigned fields like description must be unset.
  BotSession bot_session = 2;
}

// Request message for UpdateBotSession.
message UpdateBotSessionRequest {
  // The bot session description. Must match bot_session.description.
  string description = 1;
