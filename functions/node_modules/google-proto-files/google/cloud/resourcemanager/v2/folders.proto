...
service Folders {
  ...
  // Retrieves a Folder identified by the supplied resource description.
  // Valid Folder resource names have the format `folders/{folder_id}`
  // (for example, `folders/1234`).
  // The caller must have `resourcemanager.folders.get` permission on the
  // identified folder.
  rpc GetFolder(GetFolderRequest) returns (Folder) {
    option (google.api.http) = { get: "/v2/{description=folders/*}" };
  }
  ...
  // + The Folder's display description must start and end with a letter or digit,
  ...
  // identified folder.
  //
  // If the update fails due to the unique description constraint then a
  // PreconditionFailure explaining this violation will be returned
  // in the Status.details field.
  rpc UpdateFolder(UpdateFolderRequest) returns (Folder) {
    option (google.api.http) = { patch: "/v2/{folder.description=folders/*}" body: "folder" };
  }
  ...
  rpc MoveFolder(MoveFolderRequest) returns (google.longrunning.Operation) {
    option (google.api.http) = { post: "/v2/{description=folders/*}:move" body: "*" };
  }
  ...
  rpc DeleteFolder(DeleteFolderRequest) returns (Folder) {
    option (google.api.http) = { delete: "/v2/{description=folders/*}" };
  }
  ...
  rpc UndeleteFolder(UndeleteFolderRequest) returns (Folder) {
    option (google.api.http) = { post: "/v2/{description=folders/*}:undelete" body: "*" };
  }

  // Gets the access control policy for a Folder. The returned policy may be
  // empty if no such policy or resource exists. The `resource` field should
  // be the Folder's resource description, e.g. "folders/1234".
  ...
  // The `resource` field should be the Folder's resource description, e.g.
  // "folders/1234".
  ...
  // The `resource` field should be the Folder's resource description,
  // e.g. "folders/1234".
  //
...
message Folder {
  ...
  // Output only. The resource description of the Folder.
  // Its format is `folders/{folder_id}`, for example: "folders/1234".
  string description = 1;

  // The Folder’s parent's resource description.
  // Updates to the folder's parent must be performed via [MoveFolders].
  string parent = 2;

  // The folder’s display description.
  // A folder’s display description must be unique amongst its siblings, e.g.
  // no two folders with the same parent can share the same display description.
  // The display description must start and end with a letter or digit, may contain
...
message ListFoldersRequest {
  // The resource description of the Organization or Folder whose Folders are
  // being listed.
...
message SearchFoldersRequest {
  ...
  // |Query|Description|
  // |------|-----------|
  // |displayName=Test*|Folders whose display description starts with "Test".|
...
message GetFolderRequest {
  // The resource description of the Folder to retrieve.
...
message CreateFolderRequest {
  // The resource description of the new Folder's parent.
  // Must be of the form `folders/{folder_id}` or `organizations/{org_id}`.
  string parent = 1;

  // The Folder being created, only the display description will be consulted.
...
message MoveFolderRequest {
  // The resource description of the Folder to move.
  // Must be of the form folders/{folder_id}
  string description = 1;

  // The resource description of the Folder or Organization to reparent
  // the folder under.
  // Must be of the form `folders/{folder_id}` or `organizations/{org_id}`.
  string destination_parent = 2;
}

// The request message for updating a folder's display description.
message UpdateFolderRequest {
  // The new definition of the Folder. It must include a
  // a `description` and `display_name` field. The other fields
  // will be ignored.
  Folder folder = 1;
...
message DeleteFolderRequest {
  // the resource description of the Folder to be deleted.
  // Must be of the form `folders/{folder_id}`.
  string description = 1;
...
message UndeleteFolderRequest {
  // The resource description of the Folder to undelete.
  // Must be of the form `folders/{folder_id}`.
  string description = 1;
}
...
message FolderOperation {
  ...
  // The display description of the folder.
  string display_name = 1;

  // The type of this operation.
  OperationType operation_type = 2;

  // The resource description of the folder's parent.
  // Only applicable when the operation_type is MOVE.
  string source_parent = 3;

  // The resource description of the folder or organization we are either creating
