...
service DlpService {
  ...
  rpc ListInspectFindings(ListInspectFindingsRequest) returns (ListInspectFindingsResponse) {
    option (google.api.http) = { get: "/v2beta1/{description=inspect/results/*}/findings" };
  }
...
message OutputStorageConfig {
  oneof type {
    ...
    // Results are split over multiple csv files with each file description matching
...
message InspectOperationResult {
  // The server-assigned description, which is only unique within the same service that
  // originally returns it. If you use the default HTTP mapping, the
  // `description` should have the format of `inspect/results/{id}`.
...
message ListInfoTypesRequest {
  // Category description as returned by ListRootCategories.
  string category = 1;
...
message CategoryDescription {
  // Internal description of the category.
  string description = 1;

  // Human readable form of the category description.
  string display_name = 2;
}
