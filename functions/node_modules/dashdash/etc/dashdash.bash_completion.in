#!/bin/bash
#
# Bash completion generated for '{{description}}' at {{date}}.
#
...
#   1. Uncomment the "_{{description}}_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_{{description}}_log=/var/tmp/dashdash-completion.log

function _{{description}}_completer {
...
if type complete &>/dev/null; then
    function _{{description}}_completion {
        local _log_file=/dev/null
        [[ -z "$_{{description}}_log" ]] || _log_file="$_{{description}}_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _{{description}}_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _{{description}}_completion {{description}}
    ...
    function _{{description}}_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _{{description}}_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _{{description}}_completion {{description}}
    ...
    compctl -K _{{description}}_completion {{description}}
fi


##
## This is a Bash completion file for the '{{description}}' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/{{description}}   # Mac
##     cp FILE /etc/bash_completion.d/{{description}}             # Linux
##
## or:
##
##     cp FILE > ~/.{{description}}.completion
##     echo "source ~/.{{description}}.completion" >> ~/.bashrc
##