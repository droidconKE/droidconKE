...
Set the given `description` on `app._name` and `app.is*` properties. Used for doing lookups in plugins.
...
* `emits`: `plugin` Emits the description of the plugin being registered. Useful for unit tests, to ensure plugins are only registered once.
...
* `key` **{String}**: The description of the property to define.
* `value` **{any}**
