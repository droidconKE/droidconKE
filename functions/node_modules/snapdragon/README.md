...
var res = snapdragon.compiler(ast)
  // compiler middleware, called when the description of the middleware
...
4. if a **match** is found, `pos()` is called, which returns a token with:
  - `type`: the description of the [compiler] to use
...
Renderers are run when the description of the compiler middleware matches the `type` defined on an ast `node` (which is defined in a parser).
...
* `key` **{String}**: propery description
* `val` **{any}**: property value
* `returns` **{Object}**: Returns the Parser instance for chaining.

Set parser `description` with the given `fn`
