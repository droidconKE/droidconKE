## Changelog
##### 2.5.7 - 2018.05.26
- Get rid of reserved variable description `final`, related [#400](https://github.com/zloirock/core-js/issues/400)
...
- `Function#description` no longer cache the result for correct behaviour with inherited constructors, [#296](https://github.com/zloirock/core-js/issues/296)
...
- Added fallback for `Function#description` on non-extensible functions and functions with broken `toString` conversion, [#193](https://github.com/zloirock/core-js/issues/193)
...
* Fixed V8 and FF `Array#{values, @@iterator}.description`
...
* Fixed `.description` property and `Function#toString` conversion some polyfilled methods
...
* Changed compression `client/*.min.js` options for safe `Function#description` and `Function#length`, should be fixed [#92](https://github.com/zloirock/core-js/issues/92)
...
##### 0.4.3 - 2015.01.10
* Added `Function` instances `description` property for IE9+
