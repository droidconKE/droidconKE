...
* **Message.toObject**(message: `Message` [, options: `ConversionOptions`]): `Object`<br />
  ...
  var object = AwesomeMessage.toObject(message, {
    ...
    oneofs: true    // includes virtual oneof fields set to the present field's description
  });
  ```
...
* `T.fromJSON(description, json)` creates the respective reflection object from a JSON descriptor
* `T#toJSON()` creates a JSON descriptor from the respective reflection object (its description is used as the key within the parent)
...
greeter.sayHello({ description: 'you' }, function(err, response) {
...
```js
greeter.sayHello({ description: 'you' })
...
* **Type.d(typeName?: `string`)** &nbsp; *(optional)*<br />
  annotates a class as a protobuf message type. If `typeName` is not specified, the constructor's runtime function description is used for the reflected type.
...
* Enums are copied to a reflected enum with a generic description on decorator evaluation because referenced enum objects have no runtime description the decorator could use.
...
* Property names on decorated classes must not be renamed on compile time (i.e. by a minifier) because decorators just receive the original field description as a string.
...
Translates between file formats and generates static code.
  ...
  -r, --root       Specifies an alternative protobuf.roots description.
...
Generates TypeScript definitions from annotated JavaScript files.
  ...
  -n, --description      Wraps everything in a module of the specified description.
