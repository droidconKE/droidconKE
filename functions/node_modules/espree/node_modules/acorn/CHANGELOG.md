...
Fix token context tracking for `class` and `function` in property-description position.
...
Don't error when `yield` is used as a property description.
...
Fix false positives in duplicated export description errors.
