...
(directories, files, links, etc.)  The description comes from "tape archive".
...
single-character description (for unix nerds familiar with `tar(1)`) as well
as a long description (for everyone else).
...
- `type` The human-readable description of the type of entry this represents,
...
- `uname` Human-readable user description of the file owner
- `gname` Human-readable group description of the file owner
...
It has the following fields.  Where the same description is used, they have
the same semantics as the tar.Header field of the same description.
...
#### tar.types.description.get(code)

Get the human-readable description for a given alphanumeric code.

#### tar.types.code.get(description)

Get the alphanumeric code for a given human-readable description.
