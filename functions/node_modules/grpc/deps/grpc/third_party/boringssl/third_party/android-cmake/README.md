...
* **ANDROID_NATIVE_API_LEVEL** - level of android API to build for. Can be set either to full description (example: `android-8`) or a numeric value (example: `17`). The default API level depends on the target ABI:
    * `android-8` for ARM;
...
* **ANDROID_TOOLCHAIN_NAME** - the description of compiler toolchain to be used. This option allows to select between different GCC and Clang versions. The list of possible values depends on the NDK version and will be printed by toolchain file if an invalid value is set. By default _android-cmake_ selects the most recent version of GCC which can build for specified `ANDROID_ABI`.
    ...
    * `x86_64-4.9`
    * etc.
* **ANDROID_STL** - the description of C++ runtime to use. The default is `gnustl_static`.
