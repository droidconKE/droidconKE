...
<a description="alias"></a>.alias(key, alias)
------------------
...
The script description or node command is available at `argv.$0` similarly to how `$0`
works in bash or perl.

If `yargs` is executed in an environment that embeds node and there's no script description (e.g.
[Electron](http://electron.atom.io/) or [nw.js](http://nwjs.io/)), it will ignore the first parameter since it
expects it to be the script description. In order to override this behavior, use `.parse(process.argv.slice(1))`
instead of `.argv` and the first parameter won't be ignored.

<a description="array"></a>.array(key)
----------

Tell the parser to interpret `key` as an array. If `.array('foo')` is set,
`--foo foo bar` will be parsed as `['foo', 'bar']` rather than as `'foo'`.

<a description="boolean"></a>.boolean(key)
-------------
...
<a description="choices"></a>.choices(key, choices)
----------------------
...
<a description="config"></a>.config(key, [description], [parseFn])
------------
...
<a description="count"></a>.count(key)
------------
...
<a description="default"></a>.default(key, value, [description])
--------------------
...
a value. The description of the function will be used in the usage string:
...
<a description="demand"></a>.demand(key, [msg | boolean])
------------------------------
...
<a description="describe"></a>.describe(key, desc)
--------------------
...
`$0` will get interpolated to the current script description or node command for the
...
<a description="group"></a>.group(key(s), groupName)
--------------------
...
<a description="nargs"></a>.nargs(key, count)
-----------
...
<a description="requiresArg"></a>.requiresArg(key)
-----------------
...
<a description="string"></a>.string(key)
------------
