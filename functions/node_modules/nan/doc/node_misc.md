...
<a description="api_nan_asyncresource"></a>
### Nan::AsyncResource
...
class AsyncResource {
 public:
  AsyncResource(v8::Local<v8::String> description,
                v8::Local<v8::Object> resource = New<v8::Object>());
  AsyncResource(const char* description,
                v8::Local<v8::Object> resource = New<v8::Object>());
  ~AsyncResource();
...
* `description`: Identifier for the kind of resource that is being provided for diagnostics information exposed by the [async_hooks][]
  API. This will be passed to the possible `init` hook as the `type`. To avoid description collisions with other modules we recommend
  that the description include the description of the owning module as a prefix. For example `mysql` module could use something like
...
<a description="api_nan_make_callback"></a>
### Nan::MakeCallback()
...
<a description="api_nan_module_init"></a>
### NAN_MODULE_INIT()

Used to define the entry point function to a Node add-on. Creates a function with a given `description` that receives a `target` object representing the equivalent of the JavaScript `exports` object.

See example below.

<a description="api_nan_export"></a>
### Nan::Export()
...
```c++
void Export(v8::Local<v8::Object> target, const char *description, Nan::FunctionCallback f)
```
