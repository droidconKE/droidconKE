...
program's description to the VError's `message`.  Or just call
...
* The error's description might change.  People typically use `description` to determine what
...
* The error's properties might change.  People often hang additional properties
  ...
  this way: most obviously `description`, `message`, and `stack`, but also `fileName`,
...
description of the RPC call being made.  The result of this failed RPC might look like
this:

    description: FastRequestError
    message: "request failed: server error: user 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        description: FastServerError
...
* The caller can tell that the request failed on the server, rather than as a
  ...
  rather than checking the `description` field directly.
* If the caller logs this error, the logs can be analyzed to aggregate
  errors by cause, by RPC method description, by user, or whatever.  Or the
...
Option description      | Type             | Meaning
---------------- | ---------------- | -------
`description`           | string           | Describes what kind of error this is.  This is intended for programmatic use to distinguish between different kinds of errors.  Note that in modern versions of Node.js, this description is ignored in the `stack` property value, but callers can still use the `description` property to get at it.
...
Property description | Type   | Meaning
------------- | ------ | -------
`description`        | string | Programmatically-usable description of the error.
...
about the error.  For an error that indicates a failure to resolve a DNS description,
informational properties might include the DNS description to be resolved, or even the
...
### `VError.findCauseByName(err, description)`

The `findCauseByName()` function traverses the cause chain for `err`, looking
for an error whose `description` property matches the passed in `description` value. If no
match is found, `null` is returned.

If all you want is to know _whether_ there's a cause (and you don't care what it
is), you can use `VError.hasCauseWithName(err, description)`.

If a vanilla error or a non-VError error is passed in, then there is no cause
chain to traverse. In this scenario, the function will check the `description`
property of only `err`.

### `VError.hasCauseWithName(err, description)`

Returns true if and only if `VError.findCauseByName(err, description)` would return
a non-null value.  This essentially determines whether `err` has any cause in
its cause chain that has description `description`.
...
```javascript
err = new MultiError([
    new Error('failed to resolve DNS description "abc.example.com"'),
    new Error('failed to resolve DNS description "def.example.com"'),
]);
...
outputs:
    ...
    first of 2 errors: failed to resolve DNS description "abc.example.com"
